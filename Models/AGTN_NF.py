...
parser.add_argument('--sim', default='spring', type=str,
                    help='Choose a simulation to load, train and test')
parser.add_argument('--dim', default=None, type=int,
                    help='Input simulation dimension')
parser.add_argument('--num_epochs', default=400, type=int,
                    help='No. of training/test epochs')
parser.add_argument('--num_train_test_sim', default=200,
                    type=int, help='No. of training/test simulations')
...
def window_maker(input, nparticles, look_back_length, dim):
    ...
    for seq in range(time_period):
        ...
        if len(current_appender) >= look_back_length:
            # Note difference between appender and appenderr
            current_appenderr = [current_appender[i]
                                 for i in range(len(current_appender))]
    ...
    for seq in range(time_period):
        ...
        if len(current_appender) >= look_back_length:
            ...
            current_appenderr = [current_appender[i]
                                 for i in range(len(current_appender))]
            look_back = torch.cat(
                current_appenderr[-look_back_length:], dim=-1)
            new_appender.append(look_back)
        else:
            pass
    updated_current_state = torch.cat(new_appender, dim=0)
...
def processed(simulation, simulation_y, look_back_length, nparticles, dim):
    ...
    for sim in range(simulation.shape[0]):
        current_state = torch.cat([simulation[sim, i, :, :-2]
                                  for i in range(0, simulation.shape[1], 1)])
        current_state = window_maker(
            current_state, nparticles, look_back_length, dim)

        miscellaneous = torch.cat([simulation[sim, i, :, -2:]
                                  for i in range(look_back_length-1, simulation.shape[1], 1)])
        current_state = torch.cat([current_state, miscellaneous], dim=1)
        current_states.append(current_state)

        updated_state = torch.cat([torch.cat(
            [simulation_y[sim, i, :, :]], 1) for i in range(0, simulation.shape[1], 1)])
        updated_state = another_window_maker(
            updated_state, nparticles, look_back_length, dim, look_back_length*dim)
        updated_states.append(updated_state)

    current_states = torch.stack(current_states)
    current_states = current_states.view(-1,
                                         nparticles, current_states.shape[-1])

    updated_states = torch.stack(updated_states)
    updated_states = updated_states.view(-1,
                                         nparticles, updated_states.shape[-1])

    return current_states, updated_states


current_states, updated_states = processed(
    simulation, simulation_y, look_back_length, nparticles, dim)
...
## Create Dataset
dataset = Particle_Dataset(simulation, current_states,
                           updated_states, look_back_length)
...
ogn = OGN(dim, device, look_back_length, k_factor,
          message_passing_steps, hidden=hidden, aggr=aggr)
ogn.to(device)
print(ogn)

print(sum(p.numel() for p in ogn.parameters() if p.requires_grad))
...
# checkpoint = torch.load('{}/Autoregress_forward_{}_{}_{}_{}_non_standard.pt'.format(root_dir, sim, dim, look_back_length, train_test_sim))
...
for epoch in tqdm(range(total_epochs)):
    ...
    with torch.no_grad():
        ...
        print("Testing Loss:{}".format(
            logs['test_loss'][-1]/(look_back_length)))
    ...
    torch.save(checkpoint, 'Autoregress_GNF_G_Transformer_forward_{}_{}_{}_with_nll_final.pt'.format(sim, dim, look_back_length))
