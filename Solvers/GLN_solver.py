...
class Encoder(MessagePassing):
    def __init__(self, hidden=128, aggr='add'):
        ...
        self.enc_edge_block = edge_block(look_back_length*dim*2+1, hidden)
        self.enc_node_block = node_block(look_back_length*dim*2+2, hidden)

    def forward(self, x, edge_index):
        normalized_relative_displacements = (x[edge_index[0]][:, :-2] - x[edge_index[1]][:, :-2])
...
class InteractionNetwork(MessagePassing):
    def __init__(self, hidden=128, aggr='add'):
        ...
        self.enc_node_block = node_block(hidden*2, hidden)
...
class OGN(MessagePassing):
    ...
    def just_derivative(self, g):
        g.x = torch.autograd.Variable(g.x, requires_grad=True)
        edge_index = g.edge_index

        temporal_node_out, e_features = self.encoder(g.x, edge_index)
        
        # Lagrangian
        for gnn in self.gnn_stacks:
            temporal_node_out, e_features = gnn(temporal_node_out, edge_index, e_features)
        
        # Force_t = d(Lagrangian) w.r.t. x_t
        F_t = grad(temporal_node_out.sum(), g.x, create_graph=True)[0][:, -dim*2-2:-dim-2]
        
        q_tt = F_t/g.mass[:,-1].view(-1,1)
        F_t.retain_grad()
...
for test_sims in range(30):
    ...
    checkpoint = torch.load('{}/Original_Lagrangian_{}_{}_{}_{}_non_standard_layer_norm.pt'.format(root_dir, args.sim, args.dim, args.look_back, args.num_train_test_sim))
    ...
    base_dir = f"{root_dir}/MSE_NPJ/Lagrangian_{look_back_length}_more/{sim}"

    if not os.path.exists(base_dir):
        os.makedirs(base_dir)
    
    
    np.save("{}/MSE_NPJ/Lagrangian_{}_more/{}/Lagrangian_{}_{}_non_ln.npy".format(root_dir, look_back_length, sim, test_sims, dim), mse)
    np.save("{}/MSE_NPJ/Lagrangian_{}_more/{}/Lagrangian_test_solution_{}_{}_non_ln.npy".format(root_dir, look_back_length, sim, test_sims, dim), nn_test_solution)
    np.save("{}/MSE_NPJ/Lagrangian_{}_more/{}/Lagrangian_test_solved_{}_{}_non_ln.npy".format(root_dir, look_back_length, sim, test_sims, dim), nn_test_solved_long)
    plt.plot(range(figure_n_time_steps), mse[:figure_n_time_steps], c="blue")
    print("Done")
    plt.savefig("{}/MSE_NPJ/Lagrangian_{}_more/{}/Lagrangian_{}_{}_non_ln.png".format(root_dir, look_back_length, sim, test_sims, dim))
    plt.close()
