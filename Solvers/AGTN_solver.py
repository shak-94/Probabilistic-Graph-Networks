...
def f(x, t, conditioning=None, tenso=None):
    ...
    # predicted_next_state = np.multiply(x[:, -dim-2:-2], np.tile(std[-dim:], (nparticles,1))) + np.tile(mean[-dim:], (nparticles,1))
    # test_regress_acceleration = np.multiply(test_regress_acceleration, np.tile(accel_std[-dim:], (nparticles,1))) + np.tile(accel_mean[-dim:], (nparticles,1))
...
class MADE_Encoder(nn.Module):
    def __init__(self, dim_in, hidden, hidden_out):
        ...
        self.MADEncoder = MADEE_2(dim_in, [hidden, hidden], hidden_out, natural_ordering=True) # test nout > nin
...
class MADEE_2(nn.Module):
    ...
    def update_masks(self):
        ...
        ordering = np.arange(start=1, stop=self.nin+1,step=1) if self.natural_ordering else rng.permutation(self.nin)
...
class TransformerConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj, l_cache: OptTensor = None,
        ...
        aggr_out, e_features = self.propagate(edge_index, query=query, key=key, value=value, edge_attr=edge_attr_in, size=None, previous=l_cache)
...
class OGN(MessagePassing):
    def __init__(self, dim, device, look_back_length, k_fact, message_passing_steps, aggr='add', hidden=128):
        ...
        self.device = device
        self.dim = dim
        self.temporal_node_encoder = MADE_Encoder(dim*look_back_length, hidden=hidden, hidden_out=k_fact*dim*look_back_length) 
        self.temporal_edge_encoder = MADE_Encoder(dim*look_back_length, hidden=hidden, hidden_out=k_fact*dim*look_back_length)
    ...
    def just_derivative(self, g):
        ...
        for i in range(look_back_length):
            ...
            for gnn in self.gnn_stacks:
                temporal_node_in, e_features_in, ko = gnn(x=temporal_node_in, edge_index=edge_index, l_cache=ko, edge_attr=e_features_in, return_attention_weights=False)

            ### Decoder
            temporal_node_in = self.spatial_decoder(temporal_node_in)
            accel_out.append(temporal_node_in)
...
for test_sims in range(30):
    ...
    checkpoint = torch.load('{}/Autoregress_forward_{}_{}_{}_transformer_t_steps.pt'.format(root_dir, sim, dim, look_back_length), map_location=device)
    ...
    with torch.no_grad():
        ...
        base_dir = f"{root_dir}/MSE_NPJ/Autoregress_Gformer_{look_back_length}/{sim}"

        if not os.path.exists(base_dir):
            os.makedirs(base_dir)

        np.save("{}/MSE_NPJ/Autoregress_Gformer_{}/{}/Autoregress_Gformer_{}_{}_non_pos_no_ln_multi.npy".format(root_dir, look_back_length, sim,test_sims, dim), mse)
        np.save("{}/MSE_NPJ/Autoregress_Gformer_{}/{}/Autoregress_Gformer_test_solution_{}_{}_non_pos_no_ln_multi.npy".format(root_dir, look_back_length, sim, test_sims, dim), nn_test_solution)
        np.save("{}/MSE_NPJ/Autoregress_Gformer_{}/{}/Autoregress_Gformer_test_solved_{}_{}_non_pos_no_ln_multi.npy".format(root_dir, look_back_length, sim, test_sims, dim), nn_test_solved_long)
        plt.plot(range(figure_n_time_steps), mse[:figure_n_time_steps], c="blue")
        print("Done")
        plt.savefig("{}/MSE_NPJ/Autoregress_Gformer_{}/{}/Autoregress_Gformer_{}_{}_non_pos_no_ln_multi.png".format(root_dir, look_back_length, sim, test_sims, dim))
        plt.close()
